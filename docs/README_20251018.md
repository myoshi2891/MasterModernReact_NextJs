...
1. クライアント/サーバークライアントの不適切な使用
   ...
   - `getBooking` 関数で `supabaseBrowser` を使用
   - `getBookings` 関数で `supabaseServer` を使用
...
- `app/_lib/supabaseServer.js` を新設し、サービスロールキーを使うサーバー専用クライアントを生成
  - 環境変数の存在チェックを追加
  - ブラウザからの誤 import を防ぐガードを実装
- `app/_lib/supabaseBrowser.js` を追加
